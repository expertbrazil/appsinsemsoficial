class BillsReceife < ApplicationRecord
  include AASM
  ...
  validates :people_associated, :category, :ocorrence, :amount, :payment_form,  presence: true
  ...
  scope :receiveds, -> { where(state: :received)}
  ...
  STATES = [["Pendente", "pending", { color: "info" }], 
            ["Recebido", 'received', { color: "success" }], 
            ["Em Atraso", "overdued", { color: "warning" }], 
            ["Cancelado", 'canceled', { color: "danger" }]
           ]
...
filterrific(
  ...
  def self.options_for_sorted_by
    [
      ['Nome Filiado (a-z)', 'name_asc'],
      ['Nome Filiado (z-a)', 'name_desc'],   
      ['Plano de Contas (a-z)', 'category_name_asc'],
      ['Plano de Contas (z-a)', 'category_name_desc'],
  ...
  def self.options_for_grouped_by
    ...
    [
      ...
      ['Orgão que deverá ser cobrado', 'financial_account_id']
    ]
  end


  scope :with_situation_flag, lambda { |flag|
    where("bills_receives.state = ?", [*flag])
  }  
  ...
  scope :with_people_associated_ids, lambda { |people_associated_ids|
  ...
  scope :search_query, lambda { |query|
    ...
    where(
      terms.map {
        or_clauses = [
          "LOWER(bills_receives.name) LIKE ?"               
        ].join(' OR ')
        "(#{ or_clauses })"
      }.join(' AND '),
  ...
  scope :grouped_by, lambda { |group_option| 
    group(group_option)
  }
   ...
   scope :sorted_by, lambda { |sort_option|
    ...
    when /^due_date_/
      order("bills_receives.due_date #{ direction }")
    when /^created_at_/
      order("bills_receives.created_at #{ direction }")
    when /^name_/
  ...
  def state_name
    STATES.each do |s|
      return [s[0], s[2]] if s[1] == self.state
    end
  end
